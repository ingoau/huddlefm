...
test("links Last.fm once as a global Slack user setting", async () => {
  ...
  const request = (async (_input, init) => {
    ...
    calls.push(body);
    if (body.get("method") === "auth.getToken") return Response.json({ token: "request-token" });
    return Response.json({ session: { name: "last-user", key: "session-key" } });
  }) as typeof fetch;
  const store = new Store(":memory:");
  const dispatcher = new ScrobbleDispatcher(store, { lastFmApiKey: "api-key", lastFmSharedSecret: "secret" }, request);
  expect(await dispatcher.beginLastFm("slack-user")).toContain("token=request-token");
  expect(await dispatcher.finishLastFm("slack-user")).toBe("last-user");
  expect(store.getUserScrobbling("slack-user")).toEqual(expect.objectContaining({
    lastFmUsername: "last-user", lastFmSessionKey: "session-key", lastFmEnabled: true,
  }));
  expect(calls.map(call => call.get("method"))).toEqual(["auth.getToken", "auth.getSession"]);
  expect(calls.every(call => call.get("api_sig")?.match(/^[a-f0-9]{32}$/))).toBeTrue();
...
test("sends now playing immediately and queues scrobbles at the listening threshold", async () => {
  ...
  const request = (async (input, init) => {
    ...
    if (String(input).includes("audioscrobbler")) {
      ...
      return Response.json(method === "track.scrobble"
  ...
  playback.start({ id: "track", requesterId: "user", title: "Title", artist: "Artist", album: "Album", duration: 40 }, ["user", "bot"]);
  await until(() => calls.length === 2);
  for (const seconds of [5, 10, 15, 20]) playback.position(seconds);
  await until(() => (store.db.query("SELECT count(*) AS count FROM scrobbles WHERE status = 'sent'").get() as { count: number }).count === 2);

  const lastFmMethods = calls.filter(call => call.url.includes("audioscrobbler"))
    .map(call => new URLSearchParams(call.body).get("method"));
  const listenBrainzTypes = calls.filter(call => call.url.includes("listenbrainz"))
...
test("does not count seeks as listening time", async () => {
  ...
  store.setListenBrainzEnabled("user", true);
  const dispatcher = new ScrobbleDispatcher(store, {}, (async () => Response.json({ status: "ok" })) as unknown as typeof fetch);
  const playback = dispatcher.playback("session", "bot");
  playback.start({ id: "track", requesterId: "user", title: "Title", artist: "Artist", duration: 60 }, ["user"]);
  playback.position(5);
  playback.position(50);
  playback.position(55);
  await Bun.sleep(0);
  expect(store.db.query("SELECT count(*) AS count FROM scrobbles").get()).toEqual({ count: 0 });
  store.close();
});
...
test("one user connection applies to every Huddle playback", async () => {
  ...
  store.setListenBrainzToken("user", "token", "user");
  store.setListenBrainzEnabled("user", true);
  const dispatcher = new ScrobbleDispatcher(store, {}, (async (_input, init) => {
    if (JSON.parse(String(init?.body)).listen_type === "playing_now") playingNow++;
    return Response.json({ status: "ok" });
  }) as typeof fetch);
  const track = { id: "track", requesterId: "user", title: "Title", artist: "Artist", duration: 120 };
