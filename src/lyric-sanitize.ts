 ...
 * concatenated into one lyric line. Those role spans parse as
 * `{ startTimeMs: 0, durationMs: 0 }` after real timed syllables.
 */
function isOrphanedRolePart(part: LyricPart, sawTimedPart: boolean) {
  return sawTimedPart && part.durationMs === 0 && part.startTimeMs === 0;
}
