...
| ------------------ | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ----------------------------- | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `status`           |                                                                                                                                                                                                                                                                                                   | any grant                     | `state`, `volumePercent`, `playbackSeconds`, `displayMode`, `autoplay` (`off` / `related` / `huddle`), `loopMode` (`off` / `track` / `queue`), `transitionMode`, `duckingMode` (`off` / `gentle` / `strong`), `anchorEnabled`, `yourCapabilities` (this grant), `nowPlaying`, `queue`, `queueLimit` |
| `search`           | `query`                                                                                                                                                                                                                                                                                           | `add` or `add-bulk`           | `results`: `{ label, reference }[]`                                                                                                                                                                                                                                                                 |
| `add`              | `reference` (from search, or a media URL)                                                                                                                                                                                                                                                         | `add` / `add-bulk`            | `added`: `{ title, artist }[]`; `omitted` (count) when an album/playlist/list was truncated to the queue limit                                                                                                                                                                                      |
...
| `shuffle`          |                                                                                                                                                                                                                                                                                                   | `manage-queue`                | `shuffled` (count of songs reordered)                                                                                                                                                                                                                                                               |
...
| `settings`         | `displayMode`, `autoplay` (`off` / `related` / `huddle`, or boolean `true`=`related` / `false`=`off`), `loopMode` (`off` / `track` / `queue`, also accepted as `loop`), `transitionMode`, `duckingMode` (`off` / `gentle` / `strong`, also accepted as `ducking`), `anchorEnabled` (all optional) | `configure-settings`          | `changed`, plus current settings                                                                                                                                                                                                                                                                    |
| `end`              |                                                                                                                                                                                                                                                                                                   | `end-session`                 | `ended: true`                                                                                                                                                                                                                                                                                       |
| `release_control`  |                                                                                                                                                                                                                                                                                                   | own grant                     | `released: true`                                                                                                                                                                                                                                                                                    |

`shuffle` randomizes the pending queue. The playing track is untouched, and a queued autoplay pick keeps its slot, so only songs people asked for are reordered. It fails when fewer than two of those are queued.
...
| -------------------- | ----------------------------------------------------------- |
| `add`                | Add songs                                                   |
| `add-bulk`           | Add albums, playlists, Last.fm links, and link lists        |
| `remove-own`         | Remove songs they added                                     |
| `manage-queue`       | Manage queue                                                |
| `skip`               | Skip songs (also seek and previous)                         |
| `pause`              | Pause or resume                                             |
| `volume`             | Change volume                                               |
| `configure-settings` | Display, autoplay, loop, transitions, keep-player-at-bottom |
| `clear`              | Clear queue                                                 |
...
| `queue`          | `queue.added`, `queue.removed`, `queue.reordered`, `queue.shuffled`, `queue.cleared` |
